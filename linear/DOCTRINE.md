# Doctrine: linear (FGP Linear Daemon)

## Purpose
- Provide fast Linear operations for agent workflows.
- Reduce latency for common API queries and actions.

## Scope
- Linear API integration and FGP method surface.
- Local daemon execution and request routing.

## Non-Goals
- Full replacement for official Linear clients.
...
- Linear API calls authenticated via local credentials.

## Interfaces
- FGP methods for Linear workflows.
- CLI entrypoints via `fgp call`.

## Operational Model
- Runs locally with provider credentials.
- Owners: Linear daemon maintainers.
...
## Roadmap
- Expand coverage for additional Linear workflows.
