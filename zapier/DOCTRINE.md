# Doctrine: zapier (FGP Zapier Daemon)

## Purpose
- Provide fast Zapier operations for agent workflows.
- Reduce latency for common API queries and actions.

## Scope
- Zapier API integration and FGP method surface.
- Local daemon execution and request routing.

## Non-Goals
- Full replacement for official Zapier clients.
...
- Zapier API calls authenticated via local credentials.

## Interfaces
- FGP methods for Zapier workflows.
- CLI entrypoints via `fgp call`.

## Operational Model
- Runs locally with provider credentials.
- Owners: Zapier daemon maintainers.
...
## Roadmap
- Expand coverage for additional Zapier workflows.
