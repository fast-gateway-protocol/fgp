# Doctrine: discord (FGP Discord Daemon)

## Purpose
- Provide fast Discord operations for agent workflows.
- Reduce latency for common API queries and actions.

## Scope
- Discord API integration and FGP method surface.
- Local daemon execution and request routing.

## Non-Goals
- Full replacement for official Discord clients.
...
- Discord API calls authenticated via local credentials.

## Interfaces
- FGP methods for Discord workflows.
- CLI entrypoints via `fgp call`.

## Operational Model
- Runs locally with provider credentials.
- Owners: Discord daemon maintainers.
...
## Roadmap
- Expand coverage for additional Discord workflows.
