//! FGP daemon for OpenAI API operations.
//!
//! Provides fast access to OpenAI's chat completions, embeddings, and models API.
//!
//! # Usage
//! ```bash
//! fgp-openai start           # Start daemon in background
//! fgp-openai start -f        # Start in foreground
//! fgp-openai stop            # Stop daemon
//! fgp-openai status          # Check daemon status
//! ```
//!
//! # Authentication
...
//!
//! # Methods
//! - `openai.chat` - Chat completion with messages array
//! - `openai.complete` - Legacy text completion
//! - `openai.embed` - Generate embeddings
//! - `openai.models` - List available models
//! - `openai.health` - Check API connectivity
//!
//! # Test
//! ```bash
//! fgp call openai.models
//! fgp call openai.chat -p '{"model": "gpt-4", "messages": [{"role": "user", "content": "Hello!"}]}'
//! fgp call openai.embed -p '{"model": "text-embedding-3-small", "input": "Hello world"}'
//! ```
...
const DEFAULT_SOCKET: &str = "~/.fgp/services/openai/daemon.sock";

#[derive(Parser)]
#[command(name = "fgp-openai")]
#[command(about = "FGP daemon for OpenAI API operations")]
#[command(version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    Start {
        /// Socket path (default: ~/.fgp/services/openai/daemon.sock)
...
fn cmd_start(socket: String, foreground: bool) -> Result<()> {
    ...
    println!("Starting fgp-openai daemon...");
    println!("Socket: {}", socket_path);
    println!();
    println!("Available methods:");
    println!("  openai.chat      - Chat completion with messages");
    println!("  openai.complete  - Legacy text completion");
    println!("  openai.embed     - Generate embeddings");
    println!("  openai.models    - List available models");
    println!("  openai.health    - Check API connectivity");
    println!();
    println!("Test with:");
    println!("  fgp call openai.models");
    println!("  fgp call openai.chat -p '{{\"model\": \"gpt-4\", \"messages\": [{{\"role\": \"user\", \"content\": \"Hello!\"}}]}}'");
    println!();

    if foreground {
        // Foreground mode - initialize logging and run directly
        tracing_subscriber::fmt()
            .with_env_filter("fgp_openai=debug,fgp_daemon=debug")
            .init();

        let service = OpenAIService::new(None).context("Failed to create OpenAIService")?;
        let server =
...
fn cmd_stop(socket: String) -> Result<()> {
    ...
    if !pid_matches_process(pid, "fgp-openai") {
        anyhow::bail!("Refusing to stop PID {}: unexpected process", pid);
    }

    println!("Stopping fgp-openai daemon (PID: {})...", pid);
