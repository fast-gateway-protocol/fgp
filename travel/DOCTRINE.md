# Doctrine: travel (FGP Travel Daemon)

## Purpose
- Provide fast Travel operations for agent workflows.
- Reduce latency for common API queries and actions.

## Scope
- Travel API integration and FGP method surface.
- Local daemon execution and request routing.

## Non-Goals
- Full replacement for official Travel clients.
...
- Travel API calls authenticated via local credentials.

## Interfaces
- FGP methods for Travel workflows.
- CLI entrypoints via `fgp call`.

## Operational Model
- Runs locally with provider credentials.
- Owners: Travel daemon maintainers.
...
## Roadmap
- Expand coverage for additional Travel workflows.
