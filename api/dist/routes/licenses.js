...
licensesRoutes.post('/validate', async (c) => {
    ...
    const fingerprints = p.machineFingerprints || [];
...
licensesRoutes.get('/:key', async (c) => {
    ...
    return c.json({
        ...
        machines: purchase[0].machineFingerprints?.length || 0,
        machine_limit: 3,
...
async function logValidation(purchaseId, fingerprint, c, result) {
    if (!purchaseId)
        return;
...
async function generateDecryptionKey(licenseKey, fingerprint) {
