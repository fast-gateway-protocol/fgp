//! FGP daemon for SQLite database operations.
//!
//! Provides fast SQLite access via UNIX sockets.
//!
//! # Usage
//! ```bash
//! fgp-sqlite start           # Start daemon in background
//! fgp-sqlite start -f        # Start in foreground
//! fgp-sqlite stop            # Stop daemon
//! fgp-sqlite status          # Check daemon status
//! ```
//!
//! # Methods
//! - `sqlite.open` - Open/create a database file
//! - `sqlite.close` - Close a database connection
//! - `sqlite.query` - Execute SELECT query, return rows as JSON
//! - `sqlite.execute` - Execute INSERT/UPDATE/DELETE, return affected rows
//! - `sqlite.tables` - List tables in database
//! - `sqlite.schema` - Get schema for a table
//!
//! # Test
//! ```bash
//! fgp call sqlite.open -p '{"path": "~/data/test.db"}'
//! fgp call sqlite.tables -p '{"path": "~/data/test.db"}'
//! fgp call sqlite.query -p '{"path": "~/data/test.db", "sql": "SELECT * FROM users LIMIT 10"}'
//! fgp call sqlite.execute -p '{"path": "~/data/test.db", "sql": "INSERT INTO users (name) VALUES (?)", "params": ["Alice"]}'
//! ```
...
const DEFAULT_SOCKET: &str = "~/.fgp/services/sqlite/daemon.sock";

#[derive(Parser)]
#[command(name = "fgp-sqlite")]
#[command(about = "FGP daemon for SQLite database operations")]
#[command(version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    Start {
        /// Socket path (default: ~/.fgp/services/sqlite/daemon.sock)
...
fn cmd_start(socket: String, foreground: bool) -> Result<()> {
    ...
    println!("Starting fgp-sqlite daemon...");
    println!("Socket: {}", socket_path);
    println!();
    println!("Available methods:");
    println!("  sqlite.open     - Open/create a database file");
    println!("  sqlite.close    - Close a database connection");
    println!("  sqlite.query    - Execute SELECT query, return rows as JSON");
    println!("  sqlite.execute  - Execute INSERT/UPDATE/DELETE, return affected rows");
    println!("  sqlite.tables   - List tables in database");
    println!("  sqlite.schema   - Get schema for a table");
    println!();
    println!("Test with:");
    println!("  fgp call sqlite.open -p '{{\"path\": \"~/data/test.db\"}}'");
    println!("  fgp call sqlite.tables -p '{{\"path\": \"~/data/test.db\"}}'");
    println!();

    if foreground {
        // Foreground mode - initialize logging and run directly
        tracing_subscriber::fmt()
            .with_env_filter("fgp_sqlite=debug,fgp_daemon=debug")
            .init();
...
fn cmd_stop(socket: String) -> Result<()> {
    ...
    if !pid_matches_process(pid, "fgp-sqlite") {
        anyhow::bail!("Refusing to stop PID {}: unexpected process", pid);
    }

    println!("Stopping fgp-sqlite daemon (PID: {})...", pid);
