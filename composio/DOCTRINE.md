# Doctrine: composio (FGP Composio Daemon)

## Purpose
- Provide fast Composio operations for agent workflows.
- Reduce latency for common API queries and actions.

## Scope
- Composio API integration and FGP method surface.
- Local daemon execution and request routing.

## Non-Goals
- Full replacement for official Composio clients.
...
- Composio API calls authenticated via local credentials.

## Interfaces
- FGP methods for Composio workflows.
- CLI entrypoints via `fgp call`.

## Operational Model
- Runs locally with provider credentials.
- Owners: Composio daemon maintainers.
...
## Roadmap
- Expand coverage for additional Composio workflows.
