# Doctrine: supabase (FGP Supabase Daemon)

## Purpose
- Provide fast Supabase operations for agent workflows.
- Reduce latency for common API queries and actions.

## Scope
- Supabase API integration and FGP method surface.
- Local daemon execution and request routing.

## Non-Goals
- Full replacement for official Supabase clients.
...
- Supabase API calls authenticated via local credentials.

## Interfaces
- FGP methods for Supabase workflows.
- CLI entrypoints via `fgp call`.

## Operational Model
- Runs locally with provider credentials.
- Owners: Supabase daemon maintainers.
...
## Roadmap
- Expand coverage for additional Supabase workflows.
